...
public class SwingAnimatorBuilder implements AnimatorBuilder {
	...
	public void addHorizontalRoad(Road l, int i, int j, boolean eastToWest) {
		...
		double y = skipInit + skipRoad + i*skipRoadCar;
		Translator t = eastToWest ? new TranslatorEW(x, y, MP.roadLength, VP.elementWidth, VP.scaleFactor) // third is roadlength
				: new TranslatorWE(x, y, MP.roadLength, VP.elementWidth, VP.scaleFactor); // third is roadlength
		painter.addRoad(l,t);
	}
	...
	public void addVerticalRoad(Road l, int i, int j, boolean southToNorth) {
		...
		Translator t = southToNorth ? new TranslatorSN(x, y, MP.roadLength, VP.elementWidth, VP.scaleFactor) // third is roadlength
				: new TranslatorNS(x, y, MP.roadLength, VP.elementWidth, VP.scaleFactor); // third is roadlength
		painter.addRoad(l,t);
	}
	...
	private static class MyPainter implements SwingAnimatorPainter {
		...
		public void paint(Graphics g) {
			...
			for (Element<Light> e : lightElements) {
				if (e.x.getState().equals("Red")) {
					g.setColor(Color.RED);
				} 
				else if(e.x.getState().equals("Yellow")){
			...
			for (Element<Road> e : roadElements) {
				XGraphics.fillRect(g, e.t, 0, 0, MP.roadLength, VP.elementWidth); // fifth is roadlength
			}
